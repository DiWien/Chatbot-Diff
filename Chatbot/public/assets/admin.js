...
async function init() {
  ...
  bindTabs(); bindForms(); await Promise.all([loadDashboard(), loadConfig(), loadNutritionConfig(), loadKnowledge(), loadLogs()]);
  activateInitialTab();
}
...
function bindForms() {
  ...
  qs('#toggleNutritionKey').onclick = () => { const input = qs('#nutritionConfigForm [name="apiKey"]'); input.type = input.type === 'password' ? 'text' : 'password'; qs('#toggleNutritionKey').textContent = input.type === 'password' ? 'Show' : 'Hide'; };
  qs('#resetPrompt').onclick = () => { qs('[name="systemPrompt"]').value = defaultPrompt; };
  qs('#resetNutritionPrompt').onclick = () => { qs('#nutritionConfigForm [name="systemPrompt"]').value = nutritionDefaultPrompt; };
  ...
  qs('#testNutritionConnection').onclick = () => runAiUsageCheck('nutrition');
  ...
  qs('#nutritionConfigForm').onsubmit = async (e) => { e.preventDefault(); const body = Object.fromEntries(new FormData(e.target)); await api('/api/admin/config', { method: 'POST', body: JSON.stringify(body) }); toast('Đã lưu Nutrition AI'); loadNutritionConfig(); loadDashboard(); };
...
async function loadDashboard() { const { dashboard } = await api('/api/admin/dashboard'); const usage = dashboard.lastUsage || {}; const logs = dashboard.logs || []; const nutrition = dashboard.nutrition || {}; qs('#dashboard').innerHTML = `<div class="dash-hero"><div><p class="eyebrow">Diff Coach Operations</p><h2>AI Control Dashboard</h2><p>Giám sát Chat AI, Nutrition AI, quota, token usage, knowledge base và lỗi gần nhất trong một màn hình.</p></div><div class="dash-status ${dashboard.aiStatus === 'active' ? 'ok' : 'warn'}"><span></span>${dashboard.aiStatus === 'active' ? 'Active' : 'Inactive'}</div></div><div class="metric-grid dash-metrics">${[['Server',dashboard.serverStatus,'Online status'],['Chat AI',`${dashboard.provider} / ${dashboard.model}`,'General chat'],['Nutrition AI',`${nutrition.provider || '-'} / ${nutrition.model || '-'}`,'AI Nutrition Coach'],['Questions',dashboard.totalQuestions,'Total handled'],['Documents',dashboard.totalDocuments,'Knowledge items'],['Chunks',dashboard.totalChunks,'RAG chunks'],['Last tokens',usage.totalTokens ? `${usage.totalTokens}` : '-','Input/output usage'],['API test',dashboard.lastTestStatus,'Connection status']].map(([k,v,h])=>`<div class="metric pro"><small>${k}</small><strong>${v ?? '-'}</strong><span>${h}</span></div>`).join('')}</div><div class="dashboard-grid"><section class="card ops-card"><div class="card-head"><div><p class="eyebrow">Quota & Tokens</p><h2>AI Usage Check</h2></div><div class="actions"><button class="btn primary" type="button" id="dashboardTestConnection">Check Chat</button><button class="btn" type="button" id="dashboardNutritionTestConnection">Check Nutrition</button></div></div><div class="usage-strip"><div><small>Input</small><strong>${usage.inputTokens || 0}</strong></div><div><small>Output</small><strong>${usage.outputTokens || 0}</strong></div><div><small>Total</small><strong>${usage.totalTokens || 0}</strong></div></div><pre id="dashboardUsageCheck">Chưa kiểm tra trong phiên này.</pre></section><section class="card ops-card"><p class="eyebrow">System</p><h2>Runtime Health</h2><div class="info-list"><p><span>Version</span><strong>${dashboard.version}</strong></p><p><span>Allowed origin</span><strong>${dashboard.allowedOrigin || '-'}</strong></p><p><span>Chat test</span><strong>${dashboard.lastTestStatus} ${dashboard.lastTestAt || ''}</strong></p><p><span>Nutrition test</span><strong>${nutrition.lastTestStatus || 'never'} ${nutrition.lastTestAt || ''}</strong></p></div></section></div><section class="card recent-card"><div class="card-head"><div><p class="eyebrow">Recent Activity</p><h2>Latest Chat Logs</h2></div><button class="btn" type="button" onclick="activateTab('logs', true)">View all logs</button></div><div class="recent-list">${logs.length ? logs.map((log)=>`<div class="recent-item"><div><strong>${log.message || '-'}</strong><p>${log.source || '-'} | ${new Date(log.createdAt).toLocaleString()}</p></div><span class="log-badge ${log.responseStatus === 'success' ? 'ok' : 'err'}">${log.responseStatus}</span></div>`).join('') : '<p class="muted">Chưa có logs.</p>'}</div></section>`; qs('#dashboardTestConnection').onclick = () => runAiUsageCheck(); qs('#dashboardNutritionTestConnection').onclick = () => runAiUsageCheck('nutrition'); }
...
async function loadNutritionConfig() { const { config, defaultPrompt: dp } = await api('/api/admin/config/nutrition'); nutritionDefaultPrompt = dp; const f = qs('#nutritionConfigForm'); Object.entries(config).forEach(([k,v]) => { const el = f.elements[k]; if (el && k !== 'apiKey' && k !== 'nutrition') el.value = v ?? ''; }); qs('#nutritionApiKeyMask').textContent = config.hasApiKey ? `Đã lưu: ${config.apiKeyMask}` : 'Chưa có API key Nutrition'; }
...
async function runAiUsageCheck(profile = 'chat') { try { const form = profile === 'nutrition' ? qs('#nutritionConfigForm') : qs('#configForm'); const body = Object.fromEntries(new FormData(form)); const result = await api('/api/admin/test-connection', { method: 'POST', body: JSON.stringify(body) }); renderUsageCheck(result, profile); toast(profile === 'nutrition' ? 'Kết nối Nutrition AI thành công' : 'Kết nối AI thành công'); loadDashboard(); } catch (e) { renderUsageCheck({ status: 'failed', quota: 'unavailable', error: e.message }, profile); toast(e.message); } }
function renderUsageCheck(result, profile = 'chat') { const usage = result.usage || {}; const payload = JSON.stringify({ profile, status: result.status, quota: result.quota || (result.status === 'success' ? 'available' : 'unavailable'), inputTokens: usage.inputTokens || 0, outputTokens: usage.outputTokens || 0, totalTokens: usage.totalTokens || 0, error: result.error || '', detail: result.detail || '' }, null, 2); const configUsage = qs(profile === 'nutrition' ? '#nutritionUsageCheck' : '#usageCheck'); const dashboardUsage = qs('#dashboardUsageCheck'); if (configUsage) configUsage.textContent = payload; if (dashboardUsage) dashboardUsage.textContent = payload; }
