...
async function sendChat(e) { e.preventDefault(); const input = qs('#chatInput'); const sent = input.value.trim(); if (!sent) return; addMsg(sent, 'user'); input.value = ''; const start = performance.now(); try { const data = await api('/api/chat', { method: 'POST', body: JSON.stringify({ message: sent, source: 'admin-test' }) }); addMsg(data.reply || 'Không có phản hồi.', 'bot'); const meta = data.meta || {}; qs('#chatMeta').textContent = `Latency: ${meta.latency ?? Math.round(performance.now()-start)}ms | Provider: ${meta.provider || '-'} | Model: ${meta.model || '-'} | KB: ${meta.usedKnowledge ? 'yes' : 'no'}`; } catch (err) { addMsg(err.message || 'Không gửi được tin nhắn test.', 'bot'); qs('#chatMeta').textContent = `Failed after ${Math.round(performance.now()-start)}ms`; } }
...
async function loadLogs() { const { logs } = await api('/api/admin/logs'); qs('#logRows').innerHTML = logs.map((l) => `<tr><td>${new Date(l.createdAt).toLocaleString()}</td><td>${l.source}</td><td>${l.userId || '-'}</td><td>${l.message}</td><td>${l.responseStatus}</td><td>${l.latency}ms</td><td>${l.error || '-'}</td><td>${l.errorDetail || '-'}</td></tr>`).join('') || '<tr><td colspan="8">Chưa có logs</td></tr>'; }
