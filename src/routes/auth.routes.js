...
router.post('/login', asyncHandler(async (req, res) => {
  ...
  const validEmail = String(email || '').toLowerCase() === String(config.admin.email).toLowerCase();
  ...
  return ok(res, { admin: { email: config.admin.email } });
}));
...
router.get('/me', requireAdmin, (req, res) => ok(res, { admin: { email: req.admin.sub } }));
